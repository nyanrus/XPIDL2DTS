...
export function process(src: string): string {
  ...
  for (const [interfaceName, constant] of Object.entries(getConstants())) {
    let c = "";
    for (const [constantName, constantValue] of Object.entries(constant)) {
      c += `const ${constantName} = ${constantValue};`;
    }
    buf += `export namespace ${interfaceName} { ${c} }`;
  }
